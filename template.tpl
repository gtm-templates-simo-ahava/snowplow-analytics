...
[
  ...
  {
    "type": "GROUP",
    "name": "errorTrackingConfig",
    "displayName": "Error Tracking Configuration",
    "groupStyle": "ZIPPY_OPEN",
    "subParams": [
      {
        "type": "SELECT",
        "name": "errorTrackingType",
        "displayName": "Tracking Type",
        "selectItems": [
          {
            "value": "enableErrorTracking",
            "displayValue": "Enable Error Tracking"
          },
          {
            "value": "trackError",
            "displayValue": "Track Error"
          }
        ],
        "simpleValueType": true
      },
      {
        "type": "GROUP",
        "name": "errorTrackingManual",
        "subParams": [
          {
            "type": "SIMPLE_TABLE",
            "name": "errorTrackingParameters",
            "simpleTableColumns": [
              {
...
switch (data.eventType) {
  ...
  case 'cartTracking':
    ...
    break;
  case 'errorTracking':
    if (data.errorTrackingType === 'enableErrorTracking') { 
      args.push('enableErrorTracking'); 
    } else {
	  if (!data.errorTrackingParameters || !data.errorTrackingParameters.length) return fail('No parameters added to error tracking call.');
      paramObj = makeTableMap(data.errorTrackingParameters, 'name', 'value');
